...
const SITE_ORIGIN = "https://micert.cl";
...
export default function ArticleShare({ slug }: ArticleShareProps) {
  const [copied, setCopied] = useState(false);
  const url = `${SITE_ORIGIN}/recursos/${slug}`;
  ...
  return (
    <div className="article-share">
      <p className="article-share-title">Compartir</p>
        ...
        {items.map((item) =>
          item.href ? (
            ...
            <button
              key={item.id}
              type="button"
              className={`article-share-btn${copied && item.id === "copy" ? " is-copied" : ""}`}
      ...
      {copied && <p className="article-share-copied" aria-live="polite">¡Copiado!</p>}
    </div>
  );
}
